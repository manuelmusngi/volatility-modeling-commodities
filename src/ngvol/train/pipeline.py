...
def run_experiment(cfg: ExperimentConfig) -> TrainResult:
    ...
    n = len(df)
    test_size = max(1, int(cfg.train.test_size * n))
    ...
    logger.info("Running rolling forecasts on test set.")
    ...
    return TrainResult(metrics=metrics, y_true=y_true, y_pred=y_pred, test_index=test_df.index)
