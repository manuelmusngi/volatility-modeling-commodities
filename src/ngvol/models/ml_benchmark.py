# src/ngvol/models/ml_benchmark.py
...
class MLVolBenchmark(VolatilityModel):
    ...
    def _build_design(self, returns: np.ndarray, exog: np.ndarray | None) -> tuple[np.ndarray, np.ndarray]:
        y2 = returns ** 2
        X = []
    ...
    def forecast(self, horizon: int = 1, exog_future: np.ndarray | None = None) -> np.ndarray:
        ...
        # Roll forward naively; for a 1-step horizon this is enough.
