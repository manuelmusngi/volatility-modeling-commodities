...
class MSGARCHModel(VolatilityModel):
    """
    Simple 2-regime MS-GARCH prototype.

    - Splits sample into high/low volatility subsets.
    - Fits separate GARCH(1,1) models.
    - Approximates regime probability based on relative variances.
    """
    ...
    def forecast(self, horizon: int = 1, exog_future: np.ndarray | None = None) -> np.ndarray:
        if self._garch_low is None or self._garch_high is None or self._regime_prob is None:
            raise RuntimeError("MSGARCHModel must be fit before forecasting.")
